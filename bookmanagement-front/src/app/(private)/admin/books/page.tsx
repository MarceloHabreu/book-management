            ...
            <div className="flex flex-col md:flex-row justify-between mb-8 items-center">
                ...
                <div className="md:grid lg:grid-cols-5 gap-4 flex flex-col-reverse items-end w-full md:w-auto">
                    ...
                    <div className="md:col-span-2 lg:col-span-3 relative w-full">
                        ...
                        <input
                            value={titleFilter}
                            onChange={(e) => setTitleFilter(e.target.value)} // Passa o evento diretamente
...
const BookTable = ({ titleFilter }: { titleFilter: string }) => {
    ...
    const handleDeleteClick = (book: Book) => {
        MySwal.fire({
            ...
            if (result.isConfirmed) {
                mutate(`/admin/books?title=${debouncedTitleFilter}`);
            }
        });
    };
    ...
    const {
        ...
        error,
        isLoading,
    } = useSWR<AxiosResponse<Book[]>>(`/admin/books?title=${debouncedTitleFilter}`, (url: string) =>
        httpClient.get(url)
    );
